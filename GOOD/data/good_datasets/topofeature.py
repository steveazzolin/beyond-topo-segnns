...
class TopoFeature(InMemoryDataset):
    r"""
    ...
    def __init__(self, root: str, domain: str = 'basis', shift: str = 'no_shift', subset: str = 'train', transform=None,
        ...
        self.required_num_nodes_red = 2 # Num nodes required for class 1 # WARNING: changed from 2 to 10

        if shift == "no_shift":
            self.num_nodes_min = 8
            self.num_nodes_max = 80
            self.num_nodes_red_max = 4
        ...
        super(TopoFeature, self).__init__(root, transform, pre_transform)
    ...
    def process(self):
        ...
        for _ in range(self.num_graphs):
            ...
            if self.domain == "basis":
                num_nodes_base = random.randint(self.num_nodes_min, self.num_nodes_max)
                y = torch.tensor([[random.randint(0, 1)]], dtype=torch.float)

                if y == 0:
                    pattern = random.randint(0, 1)
                    if pattern == 0:
                        # Include >= required_num_nodes_red red nodes but do not include the motif
                        num_nodes_red = random.randint(self.required_num_nodes_red, self.num_nodes_red_max)
                    elif pattern == 1:
                        # Include the motif but < required_num_nodes_red red nodes
                        num_nodes_red = random.randint(0, self.required_num_nodes_red - 1)
                        num_nodes_base -= 5
                elif y == 1:
                    pattern = 2
                    num_nodes_red = random.randint(self.required_num_nodes_red, self.num_nodes_red_max)
                ...
                num_nodes_base = max(num_nodes_base, num_nodes_red) # Avoid that num_nodes_red > num_nodes_base
                x = torch.cat(
                    (
                        # torch.tensor([[1., 1., 1.]]).repeat(num_nodes_base - num_nodes_red, 1),
                        torch.tensor([[0., 0., 1.]]).repeat(num_nodes_base - num_nodes_red, 1), # Simplified version
                        torch.tensor([[1., 0., 0.]]).repeat(num_nodes_red, 1)
                    ),
                    dim=0
                )
            ...
            if self.graph_distribution == "BA":
                basis = nx.barabasi_albert_graph(n=num_nodes_base, m=2)
                # basis.remove_nodes_from(list(nx.isolates(basis)))
            elif self.graph_distribution == "ER":
                basis = nx.erdos_renyi_graph(n=num_nodes_base, p=0.2, directed=False)
                # basis.remove_nodes_from(list(nx.isolates(basis)))
            ...
            if y == 1 or pattern == 1:
                motif, roles_motif = eval("dircycle")(start=num_nodes_base, role_start=num_nodes_base)
                plugin = np.random.choice(num_nodes_base, 1, replace=False) # Attach point
                ...
                x = torch.cat(
                    (
                        ...
                        torch.tensor([[0., 0., 1.]]).repeat(motif.number_of_nodes(), 1), # Simplified version
                    ),
                    dim=0
                )
            else:
    ...
    def processed_file_names(self):
        return [
            ...
            f'_redmax{self.num_nodes_red_max}_requiredred{self.required_num_nodes_red}' \
    ...
    def load(dataset_root: str, domain: str= 'basis', shift: str = 'no_shift', generate: bool = False, debias: bool =False, model_name: str=None):
        ...
        if domain == "basis":
            dataset = TopoFeature(dataset_root, domain=domain)
            ood1_dataset = TopoFeature(dataset_root, domain=domain, shift="size")
            ood2_dataset = TopoFeature(dataset_root, domain=domain, shift="ER")
        elif domain == "color":
            assert False
