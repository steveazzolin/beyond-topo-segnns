...
class AIDSC1(InMemoryDataset):
    r"""
    The AIDS Dataset from TUDataset where node feature matrix has appended the value "1" for each node.
    This allows for counting the number of nodes in the graph.
    """
    ...
    def load(dataset_root: str, domain: str, shift: str = 'no_shift', generate: bool = False, debias: bool =False, model_name:str=None):
        ...
        dataset._data.x = torch.cat(
            (dataset._data.x, torch.ones((dataset._data.x.shape[0], 1), dtype=torch.float)),
            dim=1
        )
        ...
        train_dataset.metric = 'F1'
        id_val_dataset.metric = 'F1'
        id_test_dataset.metric = 'F1'
