...
class SimpleMotif(InMemoryDataset):
    r"""
        A simple graph where:
        - class 1 iff colored cycle
        - class 0 otherwise
        Intended to check whether modular GNNs can sparsify a very simple toy dataset
    """
    def __init__(self, root: str, domain: str = 'basis', shift: str = 'no_shift', subset: str = 'train', transform=None,
        ...
        super(SimpleMotif, self).__init__(root, transform, pre_transform)
    ...
    def process(self):
        ...
        for _ in range(self.num_graphs):
            ...
            if self.domain == "basis":
                ...
                x = torch.cat(
                    (
                        torch.tensor([[0., 0., 1.]]).repeat(num_nodes_base, 1),
                    ),
                    dim=0
                )
            ...
            if y == 1:
                ...
                x = torch.cat(
                    (
                        x,
                        torch.tensor([[1., 0., 1.]]).repeat(motif.number_of_nodes(), 1),
                    ),
                    dim=0
                )
            else:
    ...
    def load(dataset_root: str, domain: str= 'basis', shift: str = 'no_shift', generate: bool = False, debias: bool =False):
        ...
        if domain == "basis":
            dataset = SimpleMotif(dataset_root, domain=domain)
            ood1_dataset = SimpleMotif(dataset_root, domain=domain, shift="size")
            ood2_dataset = SimpleMotif(dataset_root, domain=domain, shift="ER")
        elif domain == "color":
            assert False
