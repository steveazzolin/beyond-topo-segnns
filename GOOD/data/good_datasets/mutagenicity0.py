...
class MUTAG0(InMemoryDataset):
    ...
    def load(dataset_root: str, domain: str, shift: str = 'no_shift', generate: bool = False, debias: bool =False):
        ...
        dataset = Mutagenicity0(dataset_root + "/Mutagenicity0/")
...
class Mutagenicity0(InMemoryDataset):
    ...
    def process(self):
        ...
        for i in range(len(graph_labels)):
            ...
            x = torch.zeros((len(node_type_lists[i]), 14))
            x.scatter_(1, torch.tensor(node_type_lists[i], dtype=torch.long).unsqueeze(1), 1)
            ...
            #     assert node_type in ({4, 1}, {4, 3}, {4, 1, 3}), node_type  # NO or NH
